...
## Current State
- **Version:** 1.19.0 (cost_avoided corrected to current Opus pricing, was overstating 3x. `storage/token_tracker.PRICING` carried a stale $15/MTok Opus rate (the retired 4.0/4.1 price); `PRICING` feeds `cost_avoided()`, emitted in `_meta` on nearly every tool response (describe_dataset/get_rows/sample_rows/aggregate/join_datasets/get_session_stats/describe_column/get_data_hotspots), so every response's Opus dollar estimate was ~3x too high. Current Opus 4.8/4.7/4.6 is $5/MTok. Fixed `claude_opus` $15 -> $5; additively added `claude_sonnet` ($3) + `claude_haiku` ($1) for the full current model set (parity with jcm). `claude_opus` + `gpt5_latest` keys unchanged in name, so 1.x wire-compatible (additive keys plus a value correction). Present-value estimator (this call at current pricing); does not touch the public token counter (tokens stored, valued at display time). No INDEX_VERSION bump, no tool add/rename. Tests: token/cost/session_stats/storage/aggregate suites green (27). Suite parity: jcm v1.108.130 (receipt) + jdoc v1.97.0 (same token_tracker fix).)
